...
- [x] Expand the verified selection to Alphabet, Amazon and Tesla with exact Bitget UTA identities, SEC issuer binding, honest per-company metric coverage and local brand marks.
...
- [x] Review and deploy the newcomer release after explicit production approval.
...
104 Python tests and 45 frontend tests, formatting/static checks, TypeScript and OpenAPI snapshot pass for the local evidence-chat work. Checking evidence shows the numerical result immediately and then requests a cached-or-new Qwen explanation without a second click. Follow-ups are a conversation bound to filing-and-condition content. Compact prompts, explanation reuse and duplicate-request guards are included. Sponsored Bitget Qwen remains first; streaming stays `untested` until an approved live probe. Auto-explain was verified with mocked responses only. No live Qwen request, commit, push or deploy was made for this work.

The deployed NVIDIA slice and the September 12 counts below remain the production/newcomer baseline.

## Earlier verification — September 12, 2026

101 Python tests, 40 frontend tests, formatting/static checks, TypeScript and the production frontend build pass for the current local tree. Public Bitget market probes returned available tickers, books and online Reality catalog identities for Alphabet, Amazon and Tesla. Their issuer-bound SEC checks returned current CY2026Q2 evidence without a Qwen request. The earlier clean-install and disposable legacy-database migration/backup rehearsal remain valid for the deployed newcomer release. The final production-built browser journey completed blank start → Apple → manual assumptions → confirmation → public SEC/Bitget refresh → retained decision → deep-link reload and immutable exports, with keyboard focus and 390px/768px checks. The public site now uses an indigo-to-violet landing page, a read-only example, local vector brand and company marks, project-specific Reviso screen images, a moving evidence preview and a protected `/app` boundary. On September 13 the reviewed release was committed, pushed and deployed to the AWS-hosted private demo after an online SQLite backup. Public-page images were added to the exact unauthenticated asset allowlist after the deployed visual check exposed their authentication challenge; the complete suite passed again and the public landing, protected app boundary, authenticated health, configured Qwen provider and persisted thesis count were verified without a Qwen request. The new six-company expansion and the user's concurrent evidence-screen edits are local and not yet deployed. The frozen newcomer live-Qwen batch remains 6/10 requests: Apple and Microsoft suggestions timed out, NVIDIA failed the exact condition contract after one repair, and both cited-question integrations passed after correcting one raw harness false negative offline. See docs/EVALUATION.md and the dated reports under docs/evaluations.
