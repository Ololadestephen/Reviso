...
104 Python tests and 45 frontend tests, formatting/static checks, TypeScript, the production build and local visual review pass for the six-stock evidence-conversation release. Commits `a9b615d` and `61a21ab` were pushed to `main` and deployed to Lightsail after an integrity-checked online SQLite backup. Production migration 004, six instrument identities, configured Bitget Qwen, persisted research, the public/protected path boundary and the live landing page were verified. No Qwen request was made during deployment. Checking evidence now shows the numerical result immediately and then requests a cached-or-new Qwen explanation without a second click; follow-ups remain bound to the saved filing and conditions. Streaming stays `untested` until an approved live probe.
