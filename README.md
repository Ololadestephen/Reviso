...
Open http://127.0.0.1:5173 or http://localhost:5173 . API documentation: http://127.0.0.1:8000/docs . Default loopback uses an explicit local identity. With `REVISO_AUTH_MODE=google`, `/app` uses Google sign-in; Google Cloud must list both `http://127.0.0.1:5173` and `http://localhost:5173` as Authorized JavaScript origins because they are different origins. The public SDK bridge needs outbound access to api.bitget.com; failures produce unavailable observations, not invented values. No private thesis is sent to Bitget.
...
Current release: backend and frontend tests pass with static/format checks, TypeScript and the Vite production build for owner-scoped research and Google sign-in. Public pages make no Qwen request. Saved research requires a Google session in public-demo mode and an explicit local identity on loopback. Shared HTTP Basic is no longer an access path. A frozen newcomer Qwen batch used 6/10 authorized requests. See [evaluation](docs/EVALUATION.md) and [quality report](docs/QUALITY.md).

## Safety and release gates

Bind the normal local mode to loopback only. The opt-in public-demo mode adds explicit HTTPS origin/host allowlists and Google sign-in. Each signed-in person has a separate research library. Do not expose the local Compose listener publicly. The five-case real-model validation is a post-fix developer-constructed set, not an untouched independent benchmark. It makes no investment-performance, general research-accuracy or production-safety claim.
