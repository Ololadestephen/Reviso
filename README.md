...
Current release: 105 Python tests and 45 frontend tests pass with static/format checks, TypeScript and the Vite production build. A disposable production-built browser path completed the five-stage Apple journey with public SEC/Bitget data, keyboard validation/focus checks, 390px and 768px layout checks, deep-link persistence and no Qwen request. The public indigo-to-violet landing page, six verified company choices, automatic cited Qwen findings, inline filing-bound conversation, local vector marks, read-only NVIDIA walkthrough, plain-language guide and protected `/app` boundary are deployed to the AWS private demo. Public pages make no Qwen request; saved research still requires Basic Auth. A migration 005 repairs experimental local conversation tables without discarding their saved threads. The frozen newcomer Qwen batch used 6/10 authorized requests: both cited-question cases passed after offline correction of one harness false negative, while the three suggestion cases exposed two provider timeouts and one post-repair local-contract rejection. See [evaluation](docs/EVALUATION.md), the [live-Qwen report](docs/evaluations/2026-09-12-newcomer-live-validation-proposal/REPORT.md), the [final browser report](docs/evaluations/2026-09-12-newcomer-final-browser.md), the [six-stock deployment report](docs/evaluations/2026-09-13-six-stock-evidence-chat-deployment.md) and the [quality report](docs/QUALITY.md).
...
Before a public launch beyond the protected hackathon demo: decide how to handle the live suggestion timeouts/contract rejection, complete a Safari/Firefox matrix and newcomer user study, and finish independent quality review. The migration/backup path, local keyboard/viewport checks, final domain and deployed six-stock journey are verified. Groq remains a replaceable fallback rather than the sponsored production route.
